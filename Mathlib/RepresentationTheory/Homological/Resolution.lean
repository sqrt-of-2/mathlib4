...
@[simps]
def monoidAlgebraFree : Type u ⥤ ModuleCat.{u} k where
  obj X := ModuleCat.of k (MonoidAlgebra k X)
  map f := ModuleCat.ofHom (MonoidAlgebra.mapDomainLinearMap k k f)
...
def compForgetAugmented.toModule : SimplicialObject.Augmented (ModuleCat.{u} k) :=
  ((Augmented.whiskering _ _).obj (monoidAlgebraFree k)).obj (compForgetAugmented G)
...
def extraDegeneracyCompForgetAugmentedToModule :
  ...
  ExtraDegeneracy.map (extraDegeneracyCompForgetAugmented G) (monoidAlgebraFree k)
...
def d (G : Type u) (n : ℕ) : k[Fin (n + 1) → G] →ₗ[k] k[Fin n → G] :=
  (Finsupp.lift k[Fin n → G] k (Fin (n + 1) → G) fun g =>
    (@Finset.univ (Fin (n + 1)) _).sum fun p =>
      MonoidAlgebra.single (g ∘ p.succAbove) ((-1 : k) ^ (p : ℕ))) ∘ₗ
...
theorem d_of {n : ℕ} (c : Fin (n + 1) → G) :
    d k G n (MonoidAlgebra.single c 1) =
      Finset.univ.sum fun p : Fin (n + 1) =>
        MonoidAlgebra.single (c ∘ p.succAbove) ((-1 : k) ^ (p : ℕ)) := by
  simp [d]

lemma d_single {n : ℕ} (c : Fin (n + 1) → G) (r : k) :
    d k G n (MonoidAlgebra.single c r) =
      Finset.univ.sum fun p : Fin (n + 1) =>
        MonoidAlgebra.single (c ∘ p.succAbove) (r * (-1 : k) ^ (p : ℕ)) := by
